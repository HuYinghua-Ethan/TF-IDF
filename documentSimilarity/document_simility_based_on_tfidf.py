...
每篇文章计算一个向量，根据每篇文章的重要词的在该文章中出现的频率，作为该文章的向量
"""
...
def load_data(path):
    ...
    tf_idf_dict = calculate_tfidf(corpus)  # key:文档序号  value:dict key:词 value: tfidf
    topk_words = tf_idf_topk(tf_idf_dict, top=5, print_word=False)  # tf_idf_topk 返回的是字典 key:文档序号 value: [('特斯拉', 0.008689405349353219), ('汽车', 0.005864972634158598)...]
    vocab = set()
    for words_tfidf_dict in topk_words.values():
        for word, tfidf in words_tfidf_dict:
            vocab.add(word)
...
def doc_to_vec(passage, vocab):
    vector = [0] * len(vocab)  # 创建一个词表大小的列表
    passage_words = jieba.lcut(passage)
    for index, word in enumerate(vocab):
        vector[index] = passage.count(word) / len(passage_words)
    return vector    
...
def cosine_similarity(vector1, vector2):
    ...
    try:
        sqrt_y = math.sqrt(sum([y**2 for y in vector2]))
    except:
        return 0
